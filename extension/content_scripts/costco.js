(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractCostcoOrderData();
  ...
  function extractCostcoOrderData() {
    if (!window.location.href.includes('costco.com/order-details')) {
      throw new Error('Not on Costco order page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Costco', items, total, url: window.location.href };
  }
