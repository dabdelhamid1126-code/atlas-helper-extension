(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractTargetOrderData();
  ...
  function extractTargetOrderData() {
    if (!window.location.href.includes('/orders/details')) {
      throw new Error('Not on Target order page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Target', items, total, url: window.location.href };
  }
