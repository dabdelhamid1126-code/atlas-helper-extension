(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractHomeDepotOrderData();
  ...
  function extractHomeDepotOrderData() {
    if (!window.location.href.includes('homedepot.com/myaccount/order')) {
      throw new Error('Not on Home Depot order page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Home Depot', items, total, url: window.location.href };
  }
