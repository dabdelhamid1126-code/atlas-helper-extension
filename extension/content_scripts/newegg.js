(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractNeweggOrderData();
  ...
  function extractNeweggOrderData() {
    if (!window.location.href.includes('newegg.com/orders')) {
      throw new Error('Not on Newegg order page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Newegg', items, total, url: window.location.href };
  }
