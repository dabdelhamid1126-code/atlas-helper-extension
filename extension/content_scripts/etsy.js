(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractEtsyOrderData();
  ...
  function extractEtsyOrderData() {
    if (!window.location.href.includes('etsy.com/your/purchases')) {
      throw new Error('Not on Etsy purchases page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Etsy', items, total, url: window.location.href };
  }
