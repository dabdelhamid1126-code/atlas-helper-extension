(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractWootOrderData();
  ...
  function extractWootOrderData() {
    if (!window.location.href.includes('/Orders/Details')) {
      throw new Error('Not on Woot order page');
    }
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Woot', items, total, url: window.location.href };
  }
