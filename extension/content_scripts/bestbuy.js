(function() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'scrapeOrder') {
      try {
        const orderData = extractBestBuyOrderData();
  ...
  function extractBestBuyOrderData() {
    if (!window.location.href.includes('/orders/details')) {
      throw new Error('Not on Best Buy order page');
    }
    
    const orderNumber = document.querySelector('.order-number')?.textContent?.replace(/[^0-9]/g, '') || 'Unknown';
    ...
    return { orderNumber, orderDate: parseDate(orderDate), retailer: 'Best Buy', items, total, url: window.location.href };
  }
